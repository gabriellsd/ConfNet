...
// Função para expandir/recolher filiais de uma empresa
...
function previousStep() {
    ...
    console.log('Voltando do passo:', currentStepId);
    
    // Esconder passo atual
    currentStep.classList.remove('active');
    
    // Lógica específica para cada passo
    if (currentStepId === 'step-5-wifi') {
        // Do passo WiFi (step-5), voltar para resumo (step-4)
        const summaryStep = document.getElementById('step-4');
        if (summaryStep) {
            summaryStep.classList.add('active');
            showConfigSummary();
        }
    } else if (currentStepId === 'step-6') {
        // Do passo final, verificar se veio do WiFi ou Ethernet
        if (currentProxyConfig.connectionType === 'wifi' && currentProxyConfig.wifiNetwork) {
            // Veio do WiFi, voltar para passo WiFi
            const wifiStep = document.getElementById('step-5-wifi');
            if (wifiStep) {
                wifiStep.classList.add('active');
            }
        } else {
            // Veio do Ethernet, voltar para resumo
            const summaryStep = document.getElementById('step-4');
            if (summaryStep) {
                summaryStep.classList.add('active');
                showConfigSummary();
        ...
        // Para outros passos, usar lógica padrão
        const stepNumber = parseInt(currentStepId.replace('step-', ''));
        if (stepNumber > 1) {
...
function selectConnectionType(connectionType) {
    ...
    // Ir para o próximo passo (resumo) independente do tipo de conexão
    setTimeout(() => {
        nextStep();
    }, 500);
}

// Função para ir para o passo de escaneamento WiFi
function goToWifiScanStep() {
    console.log('Navegando para o passo de escaneamento WiFi');
    ...
    // Mostrar passo de WiFi
    const wifiStep = document.getElementById('step-5-wifi');
    if (wifiStep) {
        wifiStep.classList.add('active');
        
        // Iniciar escaneamento automaticamente
        showWifiNetworkSelection();
    }
}

// Função para mostrar seleção de rede WiFi
async function showWifiNetworkSelection() {
    try {
        console.log('Escaneando redes WiFi disponíveis...');
        
        // Mostrar loading
        const wifiContainer = document.getElementById('wifi-scan-container');
        wifiContainer.innerHTML = `
            <div class="wifi-scanning">
                <div class="loading-spinner"></div>
                <h4>Escaneando redes WiFi disponíveis...</h4>
                <p>Aguarde enquanto procuramos por redes sem fio.</p>
            </div>
        `;
        
        // Escanear redes WiFi
        const wifiResult = await window.ConfNet.scanWifiNetworks();
        
        if (wifiResult.success && wifiResult.networks && wifiResult.networks.length > 0) {
            // Mostrar lista de redes WiFi
            let wifiHtml = `
                <div class="wifi-networks-container">
                    <h4>Selecione a rede WiFi:</h4>
                    <div class="wifi-networks-list">
            `;
            
            wifiResult.networks.forEach((network, index) => {
                const signalIcon = getWifiSignalIcon(network.signal);
                const securityIcon = getWifiSecurityIcon(network.authentication);
                
                wifiHtml += `
                    <div class="wifi-network-item" onclick="selectWifiNetwork('${network.ssid}', '${network.authentication}', '${network.encryption}')">
                        <div class="wifi-network-info">
                            <div class="wifi-network-name">${network.ssid}</div>
                            <div class="wifi-network-details">
                                <span class="wifi-auth">${network.authentication}</span>
                                <span class="wifi-encryption">${network.encryption}</span>
                                <span class="wifi-type">${network.type === 'Saved' ? 'Salva' : 'Disponível'}</span>
                            </div>
                        </div>
                        <div class="wifi-network-indicators">
                            <span class="wifi-signal">${signalIcon}</span>
                            <span class="wifi-security">${securityIcon}</span>
                        </div>
                    </div>
                `;
            });
            ...
            wifiHtml += `
                    ...
                    <div class="wifi-actions">
                        <button class="back-btn" onclick="backToConnectionSelection()">Voltar</button>
                        <button class="refresh-wifi-btn" onclick="refreshWifiNetworks()">🔄 Atualizar Lista</button>
                    </div>
                </div>
            `;
            
            wifiContainer.innerHTML = wifiHtml;
            
        } else {
            // Nenhuma rede encontrada ou erro
            wifiContainer.innerHTML = `
                <div class="wifi-no-networks">
                    <div class="no-networks-icon">📶</div>
                    <h4>Nenhuma rede WiFi encontrada</h4>
                    <p>${wifiResult.message || 'Não foi possível encontrar redes WiFi disponíveis.'}</p>
                    <div class="wifi-actions">
                        <button class="back-btn" onclick="backToConnectionSelection()">Voltar</button>
                        <button class="refresh-wifi-btn" onclick="refreshWifiNetworks()">🔄 Tentar Novamente</button>
                        <button class="debug-wifi-btn" onclick="debugWifiScan()">🔧 Debug</button>
                    </div>
                </div>
            `;
        }
        ...
        wifiContainer.innerHTML = `
            <div class="wifi-error">
                ...
                <h4>Erro ao escanear redes WiFi</h4>
                <p>Ocorreu um erro ao tentar encontrar redes WiFi disponíveis.</p>
...
// Função para selecionar uma rede WiFi específica
async function selectWifiNetwork(ssid, authentication, encryption) {
    console.log(`Rede WiFi selecionada: ${ssid}`);
    
    // Marcar rede selecionada visualmente
    ...
    const wifiContainer = document.getElementById('wifi-scan-container');
    wifiContainer.innerHTML = `
        <div class="wifi-connecting">
            <div class="loading-spinner"></div>
            <h4>Conectando à rede WiFi</h4>
            <p>Conectando à rede <strong>${ssid}</strong> com credenciais padrão...</p>
            <div class="connection-details">
                <div class="detail-item">
                    <strong>SSID:</strong> ${ssid}
                </div>
                <div class="detail-item">
                    <strong>Segurança:</strong> ${authentication}
                </div>
                <div class="detail-item">
                    <strong>Status:</strong> Conectando...
                </div>
            </div>
        </div>
    `;
    ...
    try {
        // Tentar conectar à rede com senha padrão
        const connectionResult = await window.ConfNet.connectToWifi(ssid, '90op()OP');
        
        if (connectionResult.success) {
            console.log('✅ Conectado com sucesso à rede WiFi');
            
            // Salvar informações da rede WiFi conectada
            currentProxyConfig.wifiNetwork = {
                ssid: ssid,
                authentication: authentication,
                encryption: encryption,
                connected: true,
                connectedSSID: connectionResult.connectedSSID
            };
            ...
            wifiContainer.innerHTML = `
                <div class="wifi-connected">
                    ...
                    <p>Conectado à rede <strong>${ssid}</strong></p>
                    <div class="connection-details">
                        <div class="detail-item success">
                            <strong>Status:</strong> Conectado
                        </div>
                        <div class="detail-item">
                            <strong>SSID:</strong> ${connectionResult.connectedSSID || ssid}
                        </div>
                        <div class="detail-item">
                            <strong>Segurança:</strong> ${authentication}
                        </div>
                    </div>
                    <p class="auto-continue">Finalizando configuração automaticamente...</p>
                </div>
            `;
            
            // Finalizar automaticamente após 2 segundos
            setTimeout(() => {
                goToFinalStep();
            }, 2000);
            
        } else {
            console.log('❌ Falha ao conectar à rede WiFi');
            
            // Mostrar erro de conexão
            wifiContainer.innerHTML = `
                <div class="wifi-connection-failed">
                    <div class="error-icon">❌</div>
                    <h4>Falha na conexão</h4>
                    <p>Não foi possível conectar à rede <strong>${ssid}</strong></p>
                    <div class="error-details">
                        <p>${connectionResult.message}</p>
                        <div class="detail-item">
                            <strong>Rede atual:</strong> ${connectionResult.connectedSSID || 'Nenhuma'}
                        </div>
                    </div>
                    <div class="wifi-actions">
                        <button class="retry-btn" onclick="selectWifiNetwork('${ssid}', '${authentication}', '${encryption}')">🔄 Tentar Novamente</button>
                        <button class="back-btn" onclick="refreshWifiNetworks()">Escolher Outra Rede</button>
                    </div>
                </div>
            `;
        }
        ...
        console.error('Erro ao conectar à rede WiFi:', error);
        
        // Mostrar erro crítico
        wifiContainer.innerHTML = `
            <div class="wifi-connection-error">
                <div class="error-icon">⚠️</div>
                <h4>Erro na conexão</h4>
                <p>Ocorreu um erro ao tentar conectar à rede <strong>${ssid}</strong></p>
                <div class="error-details">${error.message}</div>
                <div class="wifi-actions">
                    <button class="retry-btn" onclick="selectWifiNetwork('${ssid}', '${authentication}', '${encryption}')">🔄 Tentar Novamente</button>
                    <button class="back-btn" onclick="refreshWifiNetworks()">Voltar</button>
                </div>
            </div>
        `;
    }
...
// Função para ir para o passo final (sucesso)
function goToFinalStep() {
    console.log('Navegando para o passo final');
    ...
    // Mostrar passo final
    const finalStep = document.getElementById('step-6');
    if (finalStep) {
        finalStep.classList.add('active');
        
        // Carregar resumo final da configuração
...
// Função para voltar à seleção de tipo de conexão
function backToConnectionSelection() {
    console.log('Voltando para seleção de tipo de conexão');
    ...
    // Mostrar passo de seleção de conexão (step-3)
    const connectionStep = document.getElementById('step-3');
    if (connectionStep) {
        connectionStep.classList.add('active');
    }
    
    // Limpar seleção de WiFi
    if (currentProxyConfig.wifiNetwork) {
        delete currentProxyConfig.wifiNetwork;
    }
    
    // Limpar seleção de tipo de conexão
    currentProxyConfig.connectionType = null;
    document.querySelectorAll('.connection-btn').forEach(btn => {
        btn.classList.remove('selected');
    });
}

// Função para atualizar lista de redes WiFi
async function refreshWifiNetworks() {
    await showWifiNetworkSelection();
}

// Função para debug do escaneamento WiFi
async function debugWifiScan() {
    const wifiContainer = document.getElementById('wifi-scan-container');
    
    wifiContainer.innerHTML = `
        <div class="wifi-debug">
            <h4>🔧 Debug do Escaneamento WiFi</h4>
            <div class="debug-info">
                <p>Testando diferentes métodos de escaneamento...</p>
                <div class="debug-results" id="debug-results">
                    <div class="debug-step">
                        <strong>1. Testando comando netsh básico...</strong>
                        <div class="debug-output" id="debug-step-1">Executando...</div>
                    </div>
                </div>
            </div>
            <div class="wifi-actions">
                <button class="back-btn" onclick="refreshWifiNetworks()">Voltar ao Escaneamento</button>
            </div>
        </div>
    `;
    
    try {
        // Teste detalhado dos comandos WiFi
        const debugStep1 = document.getElementById('debug-step-1');
        debugStep1.innerHTML = 'Executando testes detalhados...';
        
        const debugResult = await window.ConfNet.debugWifiCommands();
        
        const debugResults = document.getElementById('debug-results');
        debugResults.innerHTML = ''; // Limpar conteúdo anterior
        
        if (debugResult.success) {
            const results = debugResult.debugResults;
            
            // 1. Teste de adaptadores WiFi
            debugResults.innerHTML += `
                <div class="debug-step">
                    <strong>1. Adaptadores WiFi encontrados:</strong>
                    <div class="debug-output">${formatDebugResult(results.adapterCheck)}</div>
                </div>
            `;
            
            // 2. Saída bruta do comando profiles
            debugResults.innerHTML += `
                <div class="debug-step">
                    <strong>2. Comando 'netsh wlan show profiles' (saída bruta):</strong>
                    <div class="debug-output">${formatDebugResult(results.profilesRaw)}</div>
                </div>
            `;
            
            // 3. Saída bruta do comando networks
            debugResults.innerHTML += `
                <div class="debug-step">
                    <strong>3. Comando 'netsh wlan show networks' (saída bruta):</strong>
                    <div class="debug-output">${formatDebugResult(results.networksRaw)}</div>
                </div>
            `;
            
            // 4. Lista de todas as interfaces
            debugResults.innerHTML += `
                <div class="debug-step">
                    <strong>4. Todas as interfaces de rede:</strong>
                    <div class="debug-output">${formatDebugResult(results.interfacesList)}</div>
                </div>
            `;
            ...
            debugResults.innerHTML += `
                <div class="debug-step">
                    <strong>5. Informações do sistema:</strong>
...
Online: ${navigator.onLine}
            ...
            debugResults.innerHTML = `
                <div class="debug-step">
                    <strong>Erro no debug:</strong>
                    <div class="debug-output" style="color: red;">${debugResult.error}</div>
                </div>
            `;
        }
        ...
        document.getElementById('debug-results').innerHTML = `
            <div class="debug-step">
                ...
                <div class="debug-output" style="color: red;">${error.message}</div>
            </div>
        `;
    }
}

// Função auxiliar para formatar resultados de debug
...
// Função auxiliar para obter ícone do sinal WiFi
function getWifiSignalIcon(signal) {
    if (!signal || signal === 'N/A' || signal === 'Salvo') {
        return '📶';
    }
    
    // Tentar extrair porcentagem do sinal
...
// Função auxiliar para obter ícone de segurança WiFi
function getWifiSecurityIcon(authentication) {
    if (!authentication || authentication === 'Desconhecido') {
        return '❓';
    }
    
    const auth = authentication.toLowerCase();
    if (auth.includes('wpa') || auth.includes('wep')) {
        return '🔒';
    } else if (auth.includes('open') || auth.includes('aberto')) {
        return '🔓';
    }
...
function showConfigSummary() {
    ...
    // Adicionar informações da rede WiFi se selecionada
    if (currentProxyConfig.connectionType === 'wifi' && currentProxyConfig.wifiNetwork) {
        summaryHtml += `
            <div class="config-summary-item">
                <strong>Rede WiFi:</strong> ${currentProxyConfig.wifiNetwork.ssid}
            </div>
            <div class="config-summary-item">
                <strong>Segurança:</strong> ${currentProxyConfig.wifiNetwork.authentication}
            </div>
        `;
    }
...
function showFinalConfigSummary(networkResult = null) {
    ...
    if (currentProxyConfig.connectionType === 'ethernet') {
        ...
        // Adicionar informações da rede WiFi selecionada
        if (currentProxyConfig.wifiNetwork) {
            const securityIcon = getWifiSecurityIcon(currentProxyConfig.wifiNetwork.authentication);
            configDetails += `
                <div class="final-config-item wifi-network-selected">
                    <strong>Rede WiFi Selecionada:</strong>
                    <div class="wifi-selected-summary">
                        <div class="wifi-network-summary-item">
                            <span class="wifi-network-summary-name">${currentProxyConfig.wifiNetwork.ssid}</span>
                            <span class="wifi-network-summary-security">${securityIcon} ${currentProxyConfig.wifiNetwork.authentication}</span>
                        </div>
                    </div>
                </div>
            `;
...
async function applyProxyConfig() {
    ...
    try {
        // Esconder botões durante aplicação
        const actionButtons = document.querySelector('#step-4 .action-buttons');
        if (actionButtons) {
        ...
        if (result.success) {
            ...
            // Se for WiFi, ir para o passo de escaneamento
            if (currentProxyConfig.connectionType === 'wifi') {
                console.log('📶 Indo para passo de escaneamento WiFi...');
                goToWifiScanStep();
            } else {
                // Para Ethernet, preencher resumo final e ir para passo de sucesso
                console.log('🔌 Ethernet configurado com sucesso, indo para tela final...');
                goToFinalStep();
            }
        ...
        // Mostrar botões novamente em caso de erro
        const actionButtons = document.querySelector('#step-4 .action-buttons');
        if (actionButtons) {
